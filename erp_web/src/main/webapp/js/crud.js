	...
	$('#grid').datagrid({
		...
		toolbar: [{
			...
			handler: function(){
				...
				$.download("supplier_export.action" + listParam,formData);
			}
		},'-',{
			text: '导入',
			iconCls: 'icon-save',
			...
			buttons:[
				{
					...
					handler : function() {
						$.ajax({
							...
							data : new FormData($('#importForm')[0]), // form表单数据 
							dataType : 'json', // 把服务器响应回来的内容转成json对象
							type : 'post',//请求的方式
							processData: false, // 告诉jquery，不要处理我的数据
							contentType: false, // 告诉服务端，将来读取前端传过来的数据的时候，不要转码
							success : function(rtn) {
								$.messager.alert('提示', rtn.message, 'info',function(){
